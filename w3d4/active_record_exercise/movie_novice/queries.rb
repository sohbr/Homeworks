...
def below_average_years
  ...
  Movie.select('yr, COUNT(yr) as bad_movies').where('score < 5').group('yr').order('COUNT(yr) DESC')
...
def uma_movies
  ...
  Movie.select(:id, :title, :yr).joins(:actors).where('name = \'Uma Thurman\'').order('yr DESC')
